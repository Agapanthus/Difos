...
export class Editor {
    ...
    constructor(a: HTMLElement, c: string, callback?: ((string) =>void) ) {
        ...
        this.render(a, c, callback);

    }

    private render(a: HTMLElement, c:string, callback?: ((string) =>void) ) {
...
, 
        ...
        this.cm.on("changes", cm => {
            if(callback) callback(this.cm.getDoc().getValue());
        })
...
CodeMirror.defineOption("WYSIWYG", true, function(cm: CMEditEx, val) {
    if (val) {
